...
function offsetParent(node) {
	...
	while (isHTMLElement$1(parent) && parent.nodeName !== "HTML" && style(parent, "position") === "static") parent = parent.offsetParent;
...
function position(node, offsetParent$1) {
	...
	var offset;
	if (style(node, "position") === "fixed") offset = node.getBoundingClientRect();
	else {
		var parent = offsetParent$1 || offsetParent(node);
		offset = offset$1(node);
		if (nodeName(parent) !== "html") parentOffset = offset$1(parent);
...
function within(min$3, value, max$3) {
	return max(min$3, min(value, max$3));
}
...
function preventOverflow(_ref) {
	...
	if (checkMainAxis) {
		...
		var min$2 = offset + overflow[mainSide];
		var max$2 = offset - overflow[altSide];
		...
		var preventedOffset = within(tether ? min(min$2, tetherMin) : min$2, offset, tether ? max(max$2, tetherMax) : max$2);
...
var import_isEqual = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
...
function height(node, client) {
	var win = isWindow(node);
	return win ? win.innerHeight : client ? node.clientHeight : offset$1(node).height;
}
//#endregion
...
function getWidth(node, client) {
	var win = isWindow(node);
	return win ? win.innerWidth : client ? node.clientWidth : offset$1(node).width;
}
//#endregion
...
function getPosition(_ref) {
	...
	var _getOffset = offset$1(target), top = _getOffset.top, left = _getOffset.left, width = _getOffset.width, height = _getOffset.height;
	var _getOffset2 = offset$1(container), cTop = _getOffset2.top, cLeft = _getOffset2.left, cWidth = _getOffset2.width, cHeight = _getOffset2.height;
	var _getOffset3 = offset$1(box), bWidth = _getOffset3.width, bHeight = _getOffset3.height;
